...
import MenuItemModal from "@/app/admin/restaurants/[id]/menus/[menuNumber]/MenuItemModal";
...
export default async function DashboardMenuItemsPage({
  ...
  const { menuNumber: menuNumberStr } = await params;
  const menuNumber = Number(menuNumberStr);
  const sp = await searchParams;

  const restaurant = user.restaurant_id ? await getRestaurantById(user.restaurant_id) : null;
    ...
    <div className="min-h-screen bg-gray-100">
      <nav className="bg-white shadow-sm">
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <a href="/dashboard" className="text-sm text-amber-600 hover:text-amber-500">
            ← Volver
          </a>
        </div>
      </nav>
